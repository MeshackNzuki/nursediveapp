<template>
    <Tabs :tabs="['ATI_RN', 'HESI_RN', 'REGULAR_RN', 'CERTIFICATION_RN']">
        <template #tab-ATI_RN>
            <section
                ...
                <div class="relative flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
                    <div>
                        <h2 class="text-xl font-bold text-slate-900 dark:text-slate-100">ATI RN Test Bank</h2>
                        <p class="text-sm text-slate-600 dark:text-slate-300 mt-1">Open an ATI RN topic to load all
                            linked exam sets.</p>
                    ...
                    <input v-model="atiSearch" type="search" placeholder="Search ATI RN categories..."
            ...
            <div v-if="filteredAti.length === 0" class="text-center text-slate-400 dark:text-slate-500">No exams
                available.</div>
            ...
            <transition-group v-else name="fade" tag="div" class="grid grid-cols-1 sm:grid-cols-2 gap-4" appear>
                <article v-for="(sub, index) in filteredAti" :key="sub.slug"
                    ...
                    <div class="relative flex h-full flex-col">
                        ...
                        <p class="mt-2 text-sm text-slate-600 dark:text-slate-300">Open this category to view all
                            available ATI RN exams.</p>
        ...
        <template #tab-HESI_RN>
            <section
                ...
                <div class="relative flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
                    <div>
                        <h2 class="text-xl font-bold text-slate-900 dark:text-slate-100">HESI RN Test Bank</h2>
                        <p class="text-sm text-slate-600 dark:text-slate-300 mt-1">Open a HESI RN topic to load all
                            linked exam sets.</p>
                        <div class="mt-3 flex flex-wrap gap-2">
                            <span
                                class="rounded-2xl border border-sky-200 bg-white/85 px-3 py-1.5 text-xs font-semibold text-sky-700 dark:border-sky-700 dark:bg-slate-900/70 dark:text-sky-200">
                    ...
                    <input v-model="hesiSearch" type="search" placeholder="Search HESI RN categories..."
                        class="h-10 px-4 w-full sm:w-72 border border-cyan-300 rounded-full bg-white/90 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-400 dark:bg-slate-900/80 dark:border-cyan-700" />
                </div>
            </section>

            <div v-if="filteredHesi.length === 0" class="text-center text-slate-400 dark:text-slate-500">No exams
                available.</div>
        ...
        <template #tab-REGULAR_RN>
            <section
                ...
                <div class="relative flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
                    <div>
                        <h2 class="text-xl font-bold text-slate-900 dark:text-slate-100">Regular RN Test Bank</h2>
                        <p class="text-sm text-slate-600 dark:text-slate-300 mt-1">Open a Regular RN topic to load all
                            linked exam sets.</p>
                        <div class="mt-3 flex flex-wrap gap-2">
                            <span
                                class="rounded-2xl border border-sky-200 bg-white/85 px-3 py-1.5 text-xs font-semibold text-sky-700 dark:border-sky-700 dark:bg-slate-900/70 dark:text-sky-200">
                    ...
                    <input v-model="regularSearch" type="search" placeholder="Search Regular RN categories..."
                        class="h-10 px-4 w-full sm:w-72 border border-cyan-300 rounded-full bg-white/90 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-400 dark:bg-slate-900/80 dark:border-cyan-700" />
                </div>
            </section>

            <div v-if="filteredRegular.length === 0" class="text-center text-slate-400 dark:text-slate-500">No exams
                available.</div>
        ...
        <template #tab-CERTIFICATION_RN>
            <section
                ...
                <div class="relative flex flex-col lg:flex-row lg:items-end lg:justify-between gap-4">
                    <div>
                        <h2 class="text-xl font-bold text-slate-900 dark:text-slate-100">Certification RN Test Bank</h2>
                        <p class="text-sm text-slate-600 dark:text-slate-300 mt-1">Open a Certification RN topic to load
                            all linked exam sets.</p>
                        <div class="mt-3 flex flex-wrap gap-2">
                            <span
                                class="rounded-2xl border border-sky-200 bg-white/85 px-3 py-1.5 text-xs font-semibold text-sky-700 dark:border-sky-700 dark:bg-slate-900/70 dark:text-sky-200">
                                {{ filteredCert.length }} topics
                            </span>
                            <span
                                class="rounded-2xl border border-emerald-200 bg-emerald-50 px-3 py-1.5 text-xs font-semibold text-emerald-700 dark:border-emerald-700 dark:bg-emerald-950/60 dark:text-emerald-200">
                                {{ totalExamCount(filteredCert) }} exams
                            </span>
                        </div>
                    </div>

                    <input v-model="certSearch" type="search" placeholder="Search Certification RN categories..."
                        class="h-10 px-4 w-full sm:w-72 border border-cyan-300 rounded-full bg-white/90 text-sm focus:outline-none focus:ring-2 focus:ring-cyan-400 dark:bg-slate-900/80 dark:border-cyan-700" />
                </div>
            </section>

            <div v-if="filteredCert.length === 0" class="text-center text-slate-400 dark:text-slate-500">No exams
                available.</div>

            <transition-group v-else name="fade" tag="div" class="grid grid-cols-1 sm:grid-cols-2 gap-4" appear>
                <article v-for="(sub, index) in filteredCert" :key="sub.slug"
                    class="group relative overflow-hidden rounded-3xl border border-white/75 bg-white/90 p-5 shadow-[0_16px_42px_-30px_rgba(15,23,42,0.55)] backdrop-blur-sm transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_26px_60px_-32px_rgba(14,116,144,0.65)] dark:border-sky-800/70 dark:bg-slate-900/70 dark:hover:shadow-[0_26px_60px_-35px_rgba(6,182,212,0.75)]">
                    <div
                        :class="['absolute inset-0 opacity-0 transition-opacity duration-300 group-hover:opacity-100', accentGlowClass(index)]">
                    </div>
                    <div class="relative flex h-full flex-col">
                        ...
                        <p class="mt-2 text-sm text-slate-600 dark:text-slate-300">Open this category to view all
                            available Certification RN exams.</p>
...
const filteredAti = computed(() =>
    ati_examsubjects_rn_exams_testbank?.filter((s: any) =>
        normalizeText(s.name).toLowerCase().includes(atiSearch.value.toLowerCase())
    ) ?? []
)

const filteredHesi = computed(() =>
    hesi_examsubjects_rn_exams_testbank?.filter((s: any) =>
        normalizeText(s.name).toLowerCase().includes(hesiSearch.value.toLowerCase())
    ) ?? []
)

const filteredRegular = computed(() =>
    regular_examsubjects_rn_exams_testbank?.filter((s: any) =>
        normalizeText(s.name).toLowerCase().includes(regularSearch.value.toLowerCase())
    ) ?? []
)

const filteredCert = computed(() =>
    certification_examsubjects_rn_exams_testbank?.filter((s: any) =>
        normalizeText(s.name).toLowerCase().includes(certSearch.value.toLowerCase())
    ) ?? []
)

const accentGlowClass = (index: any) => {
    const glows = [
