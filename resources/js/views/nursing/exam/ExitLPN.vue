...
const { ati_examTopics_pn_exit_exams, hesi_examTopics_pn_exit_exams } = useNursingExamStore() as any
...
const filteredAti = computed(() => {
  if (!atiSearch.value) return ati_examTopics_pn_exit_exams ?? []
  return (ati_examTopics_pn_exit_exams ?? []).filter((e: any) =>
    normalizeText(e.name).toLowerCase().includes(atiSearch.value.toLowerCase())
  )
})

const filteredHesi = computed(() => {
  if (!hesiSearch.value) return hesi_examTopics_pn_exit_exams ?? []
  return (hesi_examTopics_pn_exit_exams ?? []).filter((e: any) =>
...
const openModal = (exam: { id: number; name: string }) => {
