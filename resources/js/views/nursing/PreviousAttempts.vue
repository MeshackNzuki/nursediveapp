<template>
    <div
        ...
        <GuestSavePrompt v-if="isGuest" product="nursing" product-label="Nursing" redirect="/nursing/previous-attempts" />
            ...
            <template v-slot:content>
                <tr v-for="(attempt, index) in filteredAttempts" :key="index">
                    <td class="p-2 whitespace-nowrap">
                        <div class="ps-1">{{ normalizeText(attempt.sub_topic_name) }}</div>
                    </td>
                    <td class="p-2 whitespace-nowrap">
                        <div class="ps-1">
                    ...
                    <td v-if="attempt.completed == 1">
                        <SmallButton icon="pi pi-refresh" classes="bg-teal-500" buttonText="Review Attempt"
                            :action="() => ReviewAttempt(attempt.id)" />
                    </td>
                    <td v-else>
                        <SmallButton icon="pi pi-refresh" class="bg-gradient-to-r from-yellow-500 to-orange-500"
                            buttonText="Resume Exam" :action="() => resumeExam(attempt.id)" />
                    </td>
...
function viewReport(attemptId: number) {
    router.push(`/nursing/performance-report/${attemptId}`)
}
