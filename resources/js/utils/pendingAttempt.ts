...
const PENDING_ATTEMPT_KEY = "nursenex_pending_attempt";

export const savePendingAttempt = (
    attempt: Omit<PendingAttempt, "createdAt">,
) => {
...
export const flushPendingAttempt = async () => {
    ...
    const response = await axios.post(
        pendingAttempt.endpoint,
        pendingAttempt.payload,
    );
    const attemptId =
        response.data?.data?.id ?? response.data?.data?.attempt_id;
