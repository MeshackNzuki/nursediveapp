        ...
        <div>
            ...
            <div v-for="(option, label) in question.options" :key="label" class="mb-2 ms-2 md:ms-8">
                <label v-if="option.choice != null && option.choice != '' && option.choice != 'None'"
                    ...
                    <input type="radio"
                        class="radio bg-blue-50 radio-neutral checked:radio-accent checked:bg-sky-50 checked:text-sky-600 mt-1 cursor-pointer border-gray-600  dark:text-slate-100 dark:border-white"
                    ...
                    <div>
                        ...
                        <span v-html="option.choice"></span> <span class="text-lg font-normal
                        ">{{ isCorrectAnswer(String(label)) && shouldShowExplanation
                            ? '&#x2713'
                            : isUserSelectedWrong(String(label)) && shouldShowExplanation
                                ? '&#10007;'
                                : '' }}</span>
                        <div v-if="shouldShowExplanation && option.reason" :class="{
                            'bg-teal-100/90 text-teal-800 p-0.5 rounded': isCorrectAnswer(String(label)), 'bg-rose-100/90 text-rose-800 p-0.5 rounded': isUserSelectedWrong(String(label))
