    ...
    <div class="h-[100dvh] overflow-y-scroll bg-white dark:bg-sky-950 dark:text-slate-300 font-roboto">
        ...
        <div v-if="examStore.questions.length > 0"
            ...
            <div v-if="examStore.testMode != 'exam'"
                :class="[showSolution ? ''
                    : '', 'flex mt-0 md:mt-24 mx-4 mb-24 flex-col gap-4 w-full md:max-w-1/2 border-l border-gray-400 border-dashed dark:border-gray-200 px-2']">
                <div class="flex items flex-col ">
                    ...
                    <span v-if="!showSolution" class="text-sm text-gray-500 mb-2">Answer the question to reveal
                        solution.</span>
                    <div :class="[!showSolution ? 'pb-24 mb-12 blur-sm bg-gray-950/25' : 'pb-24 mb-12 ']"
...
async function loadExam() {
    ...
    if (mode === 'review') {
        const { data } = await axios.get(`/teas/exam-attempts/${examId}`, { showLoader: false })
        ...
        if (resume) {
            const { data } = await axios.get(`/teas/resume-attempt/${examId}`, { showLoader: false })
            const attempt = data.data
            ...
            axios.get(`/teas/exam/${examId}`, { showLoader: false })
                .then((response) => {
                ...
                .catch((error) => {
                    ...
                    // Optionally handle quota here too if backend throws it as an error
                    let msg = error.response?.data?.error?.message
                    type.value = msg
                    if (msg == 'daily_quota_reached') {
                        showModal('pricing_action');
                    }
...
const showSolution = computed(() => {
    ...
    if (mode === 'review' || mode === 'review') return true;
